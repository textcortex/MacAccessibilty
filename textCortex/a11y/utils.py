...
from AppKit import NSWorkspace, NSApplication, NSEventMaskKeyDown, NSEvent
import signal
import objc
import Foundation
from .error import *
...
def AxUIElemnt_To_Py_Object(attrValue):
    ...
    def number_helper(number_value):
        success, int_value = CFNumberGetValue(
            number_value, kCFNumberIntType, None)
        if success:
            return int(int_value)

        success, float_value = CFNumberGetValue(
            number_value, kCFNumberDoubleType, None)
        if success:
            return float(float_value)

        raise ErrorUnsupported(
            'Error converting numeric attribute: {}'.format(number_value))
    ...
    try:
        ...
        raise ErrorUnsupported(
            'Return value not supported yet: {}'.format(ax_attr_type))
...
def get_parametrized_attribute_value(ref, attr, parameter):
    err, attrValue = AXUIElementCopyParameterizedAttributeValue(
        ref, attr, parameter, None)
    if err == kAXErrorNoValue:
        return
...
def set_app_changed_notification(call_back, key_down_callback):
    workspace = NSWorkspace.sharedWorkspace()
    center = workspace.notificationCenter()

    center.addObserverForName_object_queue_usingBlock_(
        NSWorkspaceDidActivateApplicationNotification, None, None, call_back)
    center.addObserverForName_object_queue_usingBlock_(
        NSWorkspaceDidLaunchApplicationNotification, None, None, call_back)
    # Add local monitor for key down event
    event_mask = NSEventMaskKeyDown
